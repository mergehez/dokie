        ...
        <div class="py-1 flex-1 overflow-y-auto flex flex-col">
            <div v-if="endpoint.activeRequestTab === 'body'" class="flex items-center space-x-4 px-4 py-1.5 bg-x1 border-b border-x4">
                ...
                <ElRadioGroup v-model="endpoint.request.bodyType" size="small">
                    <ElRadio v-for="item in ['json', 'xml', 'text', 'html', 'form-data'] as const" :key="item" :value="item" class="mr-4!">{{ item }} </ElRadio>
                </ElRadioGroup>
            </div>
            <ElScrollbar class="flex-1 w-full overflow-y-auto" view-class="h-full">
                ...
                <div v-if="endpoint.activeRequestTab === 'params'" class="space-y-6 p-4 h-full">
                    ...
                    <div v-if="endpoint.routeKeyVals.keyVals.length > 0" class="space-y-4">
                        ...
                        <EnvDataTable code="routeKeyVals" :kv-collection="endpoint.routeKeyVals" :on-change="endpoint.updateCurrentUrl" autocomplete />
                        <hr class="border-x4" />
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <button @click="() => endpoint.queryKeyVals.addNew()" class="px-1.5 py-0.5 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                                <i class="icon icon-[mdi--plus]"></i>
                                Add
                            </button>
                        </div>
                        <EnvDataTable code="queryKeyVals" :kv-collection="endpoint.queryKeyVals" :on-change="endpoint.updateCurrentUrl" autocomplete />
                    </div>
                </div>
                ...
                <div v-if="endpoint.activeRequestTab === 'headers'" class="space-y-4 p-4 h-full">
                    <div class="flex justify-between items-center">
                        ...
                        <button @click="() => endpoint.headerKeyVals.addNew()" class="px-1.5 py-0.5 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                            Add Header
                        </button>
                    </div>
                    <EnvDataTable code="headerKeyVals" :kv-collection="endpoint.headerKeyVals" :on-change="endpoint.updateCurrentUrl" autocomplete />
                </div>
                    ...
                    <template v-if="endpoint.request.bodyType == 'form-data'">
                        <div class="px-2 grid gap-1">
                            <div class="flex justify-between items-center px-0.5">
                                ...
                                <button @click="() => endpoint.formData.addNew()" class="px-2 py-0.5 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                                    Add
                                </button>
                            </div>
                            <EnvDataTable code="bodyFD" :kv-collection="endpoint.formData" :on-change="endpoint.updateCurrentUrl" autocomplete>
                                <template #center="{ item }">
                                    <ElSelect :model-value="item.type || 'text'" @change="(v) => (item.type = v)" style="width: 100px; height: 100%">
