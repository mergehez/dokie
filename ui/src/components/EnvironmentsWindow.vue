...
<template>
    <div class="flex gap-1 flex-col">
        ...
        <div class="flex flex-col w-full">
            <template v-for="(_, i) in globalKeyVals.hostnames" :key="i">
                <div class="w-full flex gap-1 items-center">
                    <input type="checkbox" :checked="activeHostnameIndex === i" @change="(e) => changeActiveHostname(i, (e.target as HTMLInputElement).checked)" class="size-5" />
                    <ArgInput :model-value="globalKeyVals.hostnames[i]" @update:model-value="(v) => onHostnameChange(v || '', i)" placeholder="Value" />
                </div>
            </template>
        </div>
        ...
        <div class="whitespace-nowrap w-full flex justify-between items-center text-sm font-bold mt-4">
            ...
            <button @click="() => globalKeyVals.variables.addNew()" class="px-2 py-0.5 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                <i class="icon icon-[mdi--plus]"></i>
                Add
            </button>
        </div>
        <EnvDataTable code="globals" :kv-collection="globalKeyVals.variables" :on-change="updateKeyValue" :skip="(p) => p.key === 'hostname' || p.key === 'hostnames'" />
        ...
        <div class="whitespace-nowrap w-full flex justify-between items-center pl-1 text-sm font-bold mt-4">
            ...
            <button @click="() => globalKeyVals.headers.addNew()" class="px-2 py-0.5 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                <i class="icon icon-[mdi--plus]"></i>
                Add
            </button>
        </div>
        <EnvDataTable code="globals" :kv-collection="globalKeyVals.headers" :on-change="updateKeyValue" />
    </div>
</template>
