...
function filterStrings(q: string, all: string[]) {
    ...
    if (q) {
        all = q.includes('.') ? all.filter((item) => item.toLowerCase().startsWith(q)) : all.filter((item) => item.toLowerCase().includes(q));
    }
    return all;
}
...
function suggest(q: string) {
    ...
    return (options.value = filterStrings(
        q,
        envs.variables.merge(envs.headers).map((t) => t.key)
    )
        .filter((t) => !['hostnames', 'hostname'].includes(t))
        .map((t) => {
            return {
                value: t + (useSuffix ? '}}' : ''),
                label: t,
