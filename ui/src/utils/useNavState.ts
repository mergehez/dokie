...
const createNavState = () => {
    ...
    const customEndpoints = ref(
        distinctBy(navState.custom_endpoints, (t) => t.name + t.path).map((t) => {
    ...
    const selectedEndpoints = ref(
        navState.selected_endpoints
            // .map(t => allEndpoints.find(e => e.id === t))
            .map((t) =>
                Object.values(groupedEndpoints.value)
                    .flatMap((t) => t)
                    .find((e) => e.id === t)
            )
            .filter((e) => e)
    ...
    const activeEndpoint = ref(
        Object.values(groupedEndpoints.value)
            .flatMap((t) => t)
            .find((t) => t.id == navState.active_endpoint)
    );
